<script>
	...
	export default {
		onLaunch: function() {
		    ...
		    uni.getSystemInfo({
		        success: function(e) {
					...
					// #ifdef MP
						// 小程序好像不占用底部高度
						appContext.config.globalProperties.navBarHeight=navH;
					// #endif	
					// #ifndef MP
		...
		methods:{
			...
			loadVConsole() { //初始化vConsole，用于H5调试用
				if (config.isVConsole) { //开启调试时
					...
					vConsole = new VConsole({
						...
						onClearLog: () => {
							console.info('vConsole: onClearLog');
						}
					});
				}
			},
			...
			keepStart(){
				keep.setTitle(this.$t('system.keepTitle'));
				keep.setContent(this.$t('system.keepContent'));
				keep.setSmallIcon("icon"); //图标名字  图标放在 插件下面的 res/drawable 文件夹下
				keep.onAddBackgroundCallback(function(res){
					console.info("后台运行 "+res)
					if(res==false){
						keep.setShowChannelBrage(false);// 隐藏通知角标
					}
				})
				keep.onAddScrrenListenerCallback(function(res){
					console.info("屏幕开启状态 "+res)
				})	
			...
			isRunning() { //是否正在运行
				let run=keep.isRunning();
				let liveTime=keep.getLiveTime();// 毫秒时间
				console.info(run ? "已运行："+this.$util.formatDuration(liveTime) +" ms" : "重新运行保活");
				if(!run){
					keep.restart(); 
				}
			},
				...
				uni.$on("getPositonsOrder", (res) => {
					...
					switch (res['type']) {
						...
						case "offline":
						   ...
						   let globalConfig=userStore.globalConfig;
						   // 如果开启了多设备同时登录，则不走后面的逻辑
							 if(parseInt(globalConfig.sysInfo.multipleLogin)==1){
							   break;
							 }else if(data.id==userInfo.user_id && data.client_id!=clientId && data.isMobile){
								uni.showToast({
									title: this.$t('user.offlineOut'),
									icon: "none",
									duration: 2500
								})
								userStore.logout();
			...
			checkSystemAuth(){
				userStore.systemNotify=keep.checkAppNotification();
				userStore.systemBattery=keep.isIgnoringBatteryOptimizations();
			}
		},
		onHide: function() {
