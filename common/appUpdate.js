	...
	if(data.updateType == 'forcibly' || data.updateType == 'solicit'){
		...
		dtask = plus.downloader.createDownload(data.downloadUrl, {
			...
			if (status == 200) {
				popupObj.change({
					progressValue: 100,
					progressTip:$t('app.installing'),
					progress: true,
					buttonNum: 0
				});
				plus.runtime.install(download.filename, {}, function() {
					popupObj.change({
						contentText: $t('app.updated'),
						buttonNum: 1,
						progress: false
					});
				}, function(e) {
					popupObj.cancel();
					plus.nativeUI.alert($t('app.installFail')+"[" + e.code + "]：" + e.message);
				});
			} else {
				popupObj.change({
					contentText: $t('app.downloadFail'),
					buttonNum: 1,
					progress: false
				});
			}
				...
				case 3:
					...
					if(progress - lastProgressValue >= 2){
						...
						popupObj.change({
							...
							progressTip: $t('app.downloading')+progress+"%",
							progress: true
						});
					}
					break;
		...
		dtask = plus.downloader.createDownload(data.downloadUrl, {
			...
			if (status == 200) {
				...
				plus.nativeUI.alert($t('app.downloadFail'));
			}
		});
		dtask.start();
	}
...
const updatePopup = (data, callback) => {
	...
	let popupViewContentList = [{
		...
		},
		{
			tag: 'font',
			id: 'title',
			text: $t('app.newVersion') +'：' + data.versionName,
			textStyles: {
				size: '18px',
				color: "#333",
				weight: "bold",
...
const downloadPopupDrawing =(data) => {
	...
	let progressTip = data.progressTip || $t('app.readyDown');
	let contentText = data.contentText || $t('app.waitInstallNow');
	let elementList = [
		{
			tag: 'rect', //背景色
			color: '#FFFFFF',
