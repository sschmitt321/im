<template>
		<cu-custom :isBack="true"  bgColor="bg-white">
			<template #content>{{ $t('message.emojiManage') }}（{{emojiList.length}}）</template>
			<template #right>
				...
				<uni-grid :column="5" :highlight="true" @change="change" style="padding:10rpx">
					...
					<uni-grid-item v-for="(item, index) in emojiList" :index="index+1" :key="index">
						<view class="grid-item-box">
							...
							<view class="emoji-check-box" v-if="isManage" :class="item.isCheck ? 'text-blue cuIcon-roundcheckfill' : 'cuIcon-round'"></view>
						</view>
					</uni-grid-item>
				</uni-grid>
			</scroll-view>
			<view class="btn-opt bg-white im-flex im-align-items-center im-space-between pd-10" v-if="checkList.length>0">
				<button class="cu-btn bg-green" @tap="moveEmoji()">{{ $t('message.moveTop') }}</button>
				<button class="cu-btn bg-red" @tap="delEmoji()">{{ $t('common.del') }}({{checkList.length}})</button>
			</view>
		</view>
</template>
	...
	export default {
		data() {
			return {
				emojiList:[],
				isManage:false,
				manageName:this.$t('common.tidyUp'),
				checkList:[]
			}
		},
		onLoad(option){
		...
		methods: {
						...
						tempFiles.forEach((item) => {
							if(item.size>2*1024*1024){
								return uni.showToast({
									title: this.$t('user.inputAccount')+"2MB",
									icon:'error'
								})
							}
							uni.uploadFile({
			...
			manage(){
				...
				this.manageName = this.isManage ? this.$t('common.cancel') : this.$t('common.tidyUp');
				if(!this.isManage){
					this.checkList=[];
			...
			delEmoji(){
				...
				uni.showModal({
					title: this.$t('message.comfirmDelEmoji'),
					success: (res)=>{
						if (res.confirm) {
