<template>
	<view>
		<cu-custom bgColor="bg-main-bar" :isBack="true">
			...
			<template #content>{{ $t('group.user') }}</template>
		</cu-custom>
		...
		<view style="margin-top: 104rpx;">
			<view class="cu-list menu-avatar">
				<view class="cu-item" v-for="item in userList">
					...
					<view class="content">
						<view class="text-grey">
							...
							<view v-if="item.role<3" class="cu-tag round sm" :class="item.role==1 ? 'bg-red' : 'bg-orange'">{{item.role ==1?$t('group.owner'):item.role==2?$t('group.manage'):''}}</view>
							<view v-if="item.user_id==userInfo.user_id" class="cu-tag round sm">{{ $t("user.mine") }}</view>
							
						</view>
						<view v-if="noSpeakExp(item.no_speak_time)" class="text-red text-xs">{{ $t('group.noSpeakTo') }}：{{noSpeakExp(item.no_speak_time)}}</view>
					</view>
				...
				<Empty v-if="!userList.length" :noDatatext="$t('common.SearchNone')" textcolor="#999" ></Empty>
			</view>
		</view>
					...
					<view class="cu-list menu bg-white">
						<view class="cu-item" v-if="curUser">
							<view class="content im-flex im-justify-content-center im-align-items-center">
								...
								<view v-if="curUser.role==2" class="cu-tag round sm bg-orange">{{ $t('group.manage') }}</view>
							</view>
						</view>
						<view class="cu-item"  @tap="changeOwner()" v-if="isAdmin">
							<view class="content padding-tb-sm">
								<text class="c-orange">{{ $t('group.moveManage') }}</text>
							</view>
						</view>
						<view class="cu-item"  @tap="setManage()" v-if="isAdmin">
							<view class="content padding-tb-sm">
								<text>{{curUser.role==2 ? $t('group.cancelManage') : $t('group.setManage')}}</text>
							</view>
						</view>
						<view class="cu-item"  @tap="showNoSpeak()" v-if="isAdmin">
							<view class="content padding-tb-sm">
								<text>{{ $t('group.setNoSpeak') }}</text>
							</view>
						</view>
						<view class="cu-item" @tap="removeUser()">
							<view class="content padding-tb-sm">
								<text>{{ $t('group.moveUser') }}</text>
							</view>
						</view>
						<view class="parting-line-5"></view>
						<view class="cu-item" @tap="modelName=''">
							<view class="content padding-tb-sm">
								<text class="c-red">{{ $t('common.cancel') }}</text>
							</view>
						</view>
		...
		<uni-popup ref="nospeak" type="share" safeArea backgroundColor="#fff">
			<uni-list>
				...
				<uni-list-item :title="$t('common.diy')" clickable @click="setTime(0)" >
					<template v-slot:footer>
						<uni-number-box :min="1" :max="365" :value="noSpeakDay" />&nbsp;{{ $t('common.day') }}
					</template>
				</uni-list-item>
			</uni-list>
			<view class="padding flex flex-direction mt-10">
				<button class="cu-btn bg-green lg" @tap="setNoSpeak()">{{ $t('common.ok') }}</button>
			</view>
		</uni-popup>
	</view>
</template>
	...
	export default {
		data() {
			return {
				...
				curUser:{},  //选中的用户
				noSpeakList:[{
					name:this.$t('common.tenM'),
					id:1
				},{
					name:this.$t('common.oneHour'),
					id:2
				},{
					name:this.$t('common.threeHour'),
					id:3
				},{
					name:this.$t('common.oneDay'),
					id:4
				}]
			}
		},
		...
		methods: {
			...
			removeUser(){
				this.modelName='';
				uni.showModal({
					title: this.$t('common.comfirmDelUser'),
					success: e => {
						if (e.confirm) {
			...
			changeOwner(){
				this.modelName='';
				uni.showModal({
					title: this.$t('common.comfirmManageMove'),
					success: e => {
						if (e.confirm) {
